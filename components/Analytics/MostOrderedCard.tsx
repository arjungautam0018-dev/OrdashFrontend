...
interface Row { name: string; count: number; revenue: number; category: string }
...
export default function MostOrderedCard() {
    ...
    const load = useCallback(async (f: Filter) => {
        setLoading(true);
        setSelected(null);
        const cacheKey = `analytics:most-ordered:${f}`;
        const tsKey    = `analytics:most-ordered:${f}:ts`;
        try {
            if (f !== "today") {
            ...
            const res  = await authFetch(`${API.analyticsMostOrdered}?range=${f}`);
            ...
            console.error("[MostOrderedCard]", e);
        } finally {
            setLoading(false);
        }
    }, []);
                                    ...
                                    {isSelected && (
                                        <View style={styles.detail}>
                                            <View style={styles.detailChip}>
                                                ...
                                                <Text style={styles.detailValue}>₹{row.revenue.toLocaleString()}</Text>
                                            </View>
                                            <View style={styles.detailChip}>
...
const styles = StyleSheet.create({
    ...
    barFill:         { height: "100%", backgroundColor: "#6C63FF", borderRadius: s(6) },
    barFillSelected: { backgroundColor: "#4F46E5" },
    barFillDimmed:   { backgroundColor: "#D1D5DB" },
    value:           { fontSize: sf(12), fontWeight: "700", color: "#111827", minWidth: s(36), textAlign: "right" },
    valueDimmed:     { color: "#9CA3AF" },
    detail:          { flexDirection: "row", gap: s(8), marginTop: s(4) },
    detailChip:      { flex: 1, backgroundColor: "#EDE9FE", borderRadius: s(8), padding: s(8), alignItems: "center", borderWidth: 1, borderColor: "#C4B5FD" },
    detailLabel:     { fontSize: sf(10), color: "#5B21B6", fontWeight: "500" },
    detailValue:     { fontSize: sf(13), fontWeight: "700", color: "#6C63FF", marginTop: s(2) },
